...
| Telegram | nixpkgs `mautrix-telegram` (**Python**) via a wrapper | Beeper's `sh-telegram` is still the Python bridge; bbctl invokes it python-style (`-m mautrix_telegram -c config.yaml`), so `telegramCmd` strips the `-m …` and forwards to the nixpkgs entrypoint. |
...
> Telegram is a special case: Beeper provisions the Python bridge, so bbctl calls the command as `python -m mautrix_telegram …`. If another bridge ever turns out to be Python too, copy the `telegramCmd` wrapper pattern.
